...
def text_to_ppg(text):
    # 这里需要实现将文本转换为 PPG 的逻辑
    # 由于没有直接的实现，暂时返回一个零向量作为示例
    dummy_ppg = np.zeros((100, 1280))  # 假设 PPG 的长度为 100，维度为 1024
    return dummy_ppg

def main(args):
    if args.use_text_input:
        text = input("请输入要合成的文本: ")
        ppg = text_to_ppg(text)
        args.ppg = "svc_tmp_text.ppg.npy"
        np.save(args.ppg, ppg, allow_pickle=False)
    else:
        if (args.ppg == None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wave', type=str, default="/root/autodl-tmp/so-vits-svc-5.0/svc_out.wav",
                        help="Path of raw audio.")#required=True,
    parser.add_argument('--spk', type=str, default="/root/autodl-tmp/so-vits-svc-5.0/data_svc/speaker/losl/vo_dialog_SGWLQ002_wriothesley_01.spk.npy",
    ...
    #添加是否读取whisper方法
    parser.add_argument('--use-text-input', action="store_true",
                    help="Use text input instead of audio features")
