...
def text_to_spectrogram(tts_model, text, device):
    sequence = torch.IntTensor(text_to_sequence(text, ['english_cleaners']))[None, :].to(device)
    with torch.no_grad():
        _, mel_outputs_postnet, _, _ = tts_model.inference(sequence)
...
def train(rank, args, chkpt_path, hp, hp_str):
    ...
    # 初始化TTS模型
    tts_model = Tacotron2(hp).to(device)
    tts_optim = torch.optim.AdamW(tts_model.parameters(),
                                  lr=hp.train.learning_rate, betas=hp.train.betas, eps=hp.train.eps)
